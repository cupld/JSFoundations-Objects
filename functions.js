...
function getChannelName(channel) {
  // Your code here
  //return (channel.map((channelsName) => channelsName.name))
  return channel.name;
...
function channelHasVideo(videoTitle, channel) {
  // Your code here
//  return (videoTitle.some((video) => channel.title == video ))
//channel.videos.map(video => video.title == videoTitle ? true:false)
...
function getChannelByVideoTitle(videoTitle, channels) {
  // Your code here
//return channels.find(e => e.videos.some(video => video.title === videoTitle));
return channels.find((channel) => channel.videos.some((x) => x.title === videoTitle.title))
}
...
//return channels.filter((channel) => channel.name.toLowerCase().includes(query).LowerCase() || channel.description.toLowerCase()));
}
